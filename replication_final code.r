...
#nlsy<-read.dta("maintmp.RData")
load("/Users/VyUng/Downloads/maintmp.RData")
nlsy<-x
########################
### DATA PREPARATION ###
########################
...
mahal <- matchit(formula = d_obese~d_sexf+childany+childf+age+d_urban_res+
                 ...
                 discard="control")
observed.diffmeans <- mean(nlsy$CPS_hourly_rec[nlsy$d_obese==1]) - mean(nlsy$CPS_hourly_rec[nlsy$d_obese==0])
observed.diffmeans 
treat <- nlsy$d_obese

x0 <- nlsy[,c('d_sexf','childany','childf','age','d_urban_res',
               'srvy_yr','AFQTrevised','educ','tenure','NumberEmp',
               'd_race_b','d_race_o','d_marrnever','d_marroth',
               'd_ind_ag','d_ind_for','d_ind_mining','d_ind_const','d_ind_mfrg',
               'd_ind_transp','d_ind_wtrade','d_ind_rtrade','d_ind_finance',
               ...
               'd_occ_mgmt','d_occ_tech','d_occ_admin','d_occ_svc',
               'd_occ_farming','d_occ_prodxn','d_occ_operators')]
y <- nlsy$CPS_hourly_rec
genout <- GenMatch(Tr=treat, X=x0, estimand="ATT", pop.size=15, max.generations=10, replace=TRUE, cluster=c('localhost','localhost'))
mout <- Match(Y=y, Tr=treat, X=x0, estimand="ATT", Weight.matrix=genout, replace=TRUE)
summary(mout)
mb <- MatchBalance(treat~nlsy$d_sexf+nlsy$childany+nlsy$childf+nlsy$age+nlsy$d_urban_res+
                     nlsy$srvy_yr+nlsy$AFQTrevised+nlsy$educ+nlsy$tenure+nlsy$NumberEmp+
                     nlsy$d_race_b+nlsy$d_race_o+nlsy$d_marrnever+nlsy$d_marroth+
                     nlsy$d_ind_ag+nlsy$d_ind_for+nlsy$d_ind_mining+nlsy$d_ind_const+nlsy$d_ind_mfrg+
                     nlsy$d_ind_transp+nlsy$d_ind_wtrade+nlsy$d_ind_rtrade+nlsy$d_ind_finance+
                     nlsy$d_ind_bus_svc+nlsy$d_ind_pers_svc+nlsy$d_ind_entert+nlsy$d_ind_prof_svc+
                     nlsy$d_occ_mgmt+nlsy$d_occ_tech+nlsy$d_occ_admin+nlsy$d_occ_svc+
                     nlsy$d_occ_farming+nlsy$d_occ_prodxn+nlsy$d_occ_operators,match.out=mout,nboots=500)
#Treatment effect on SubGroup: Woman 
female <- subset(nlsy, d_sexf=="1")
treat1 <- female$d_obese
x1 <- female[,c('d_sexf','childany','childf','age','d_urban_res',
                'srvy_yr','AFQTrevised','educ','tenure','NumberEmp',
                'd_race_b','d_race_o','d_marrnever','d_marroth',
                'd_ind_ag','d_ind_for','d_ind_mining','d_ind_const','d_ind_mfrg',
                'd_ind_transp','d_ind_wtrade','d_ind_rtrade','d_ind_finance',
                ...
                'd_occ_mgmt','d_occ_tech','d_occ_admin','d_occ_svc',
                'd_occ_farming','d_occ_prodxn','d_occ_operators')]
y1 <- female$CPS_hourly_rec
genout1 <- GenMatch(Tr=treat1, X=x1, estimand="ATT", pop.size=10, max.generations=10, replace=TRUE, cluster=c('localhost','localhost'))
mout1 <- Match(Y=y1, Tr=treat1, X=x1, estimand="ATT", Weight.matrix=genout1, replace=TRUE)
summary(mout1)
mb1 <- MatchBalance(treat1~female$d_sexf+female$childany+female$childf+female$age+female$d_urban_res+
                      female$srvy_yr+female$AFQTrevised+female$educ+female$tenure+female$NumberEmp+
                      female$d_race_b+female$d_race_o+female$d_marrnever+female$d_marroth+
                      female$d_ind_ag+female$d_ind_for+female$d_ind_mining+female$d_ind_const+female$d_ind_mfrg+
                      female$d_ind_transp+female$d_ind_wtrade+female$d_ind_rtrade+female$d_ind_finance+
                      female$d_ind_bus_svc+female$d_ind_pers_svc+female$d_ind_entert+female$d_ind_prof_svc+
                      female$d_occ_mgmt+female$d_occ_tech+female$d_occ_admin+female$d_occ_svc+
                      female$d_occ_farming+female$d_occ_prodxn+female$d_occ_operators, match.out=mout1, nboots=500)

#Treatment effect on subgroup: White-Female 
white_female <- subset(nlsy, d_sexf=="1" & d_race_b=="0")
treat2 <- white_female$d_obese
x2 <- white_female[,c('d_sexf','childany','childf','age','d_urban_res',
                'srvy_yr','AFQTrevised','educ','tenure','NumberEmp',
                'd_race_b','d_race_o','d_marrnever','d_marroth',
                'd_ind_ag','d_ind_for','d_ind_mining','d_ind_const','d_ind_mfrg',
                'd_ind_transp','d_ind_wtrade','d_ind_rtrade','d_ind_finance',
                ...
                'd_occ_mgmt','d_occ_tech','d_occ_admin','d_occ_svc',
                'd_occ_farming','d_occ_prodxn','d_occ_operators')]
y2 <- white_female$CPS_hourly_rec
genout2 <- GenMatch(Tr=treat2, X=x2, estimand="ATT", pop.size=50, max.generations=10, replace=TRUE, cluster=c('localhost','localhost'))
mout2 <- Match(Y=y2, Tr=treat2, X=x2, estimand="ATT", Weight.matrix=genout2, replace=TRUE)
summary(mout2)
mb2 <- MatchBalance(treat2~white_female$d_sexf+white_female$childany+childf+age+d_urban_res+
                      srvy_yr+AFQTrevised+educ+tenure+NumberEmp+
                      d_race_b+d_race_o+d_marrnever+d_marroth+
                      d_ind_ag+d_ind_for+d_ind_mining+d_ind_const+d_ind_mfrg+
                      d_ind_transp+d_ind_wtrade+d_ind_rtrade+d_ind_finance+
                      d_ind_bus_svc+d_ind_pers_svc+d_ind_entert+d_ind_prof_svc+
                      d_occ_mgmt+d_occ_tech+d_occ_admin+d_occ_svc+
                      d_occ_farming+d_occ_prodxn+d_occ_operators, match.out=mout2, nboots=500)
#Treatment effect on subgroup: Old
hist(nlsy$age)
old <- subset(nlsy, age>40)
treat3 <- old$d_obese
x3 <- old[,c('d_sexf','childany','childf','age','d_urban_res',
                      'srvy_yr','AFQTrevised','educ','tenure','NumberEmp',
                      'd_race_b','d_race_o','d_marrnever','d_marroth',
                      'd_ind_ag','d_ind_for','d_ind_mining','d_ind_const','d_ind_mfrg',
                      'd_ind_transp','d_ind_wtrade','d_ind_rtrade','d_ind_finance',
                      ...
                      'd_occ_mgmt','d_occ_tech','d_occ_admin','d_occ_svc',
                      'd_occ_farming','d_occ_prodxn','d_occ_operators')]
y3 <- old$CPS_hourly_rec
genout3 <- GenMatch(Tr=treat3, X=x3, estimand="ATT", pop.size=10, max.generations=10, caliper=0.25, replace=TRUE, cluster=c('localhost','localhost'))
mout3 <- Match(Y=y3, Tr=treat3, X=x3, estimand="ATT", Weight.matrix=genout3, caliper=0.25,replace=TRUE)
summary(mout3)

#Treatment effect on subgroup: Young
young <- subset(nlsy, age<30)
treat4 <- young$d_obese
x4 <- young[,c('d_sexf','childany','childf','age','d_urban_res',
             'srvy_yr','AFQTrevised','educ','tenure','NumberEmp',
             'd_race_b','d_race_o','d_marrnever','d_marroth',
             'd_ind_ag','d_ind_for','d_ind_mining','d_ind_const','d_ind_mfrg',
             'd_ind_transp','d_ind_wtrade','d_ind_rtrade','d_ind_finance',
             ...
             'd_occ_mgmt','d_occ_tech','d_occ_admin','d_occ_svc',
             'd_occ_farming','d_occ_prodxn','d_occ_operators')]
y4 <- young$CPS_hourly_rec
genout4 <- GenMatch(Tr=treat4, X=x4, estimand="ATT", pop.size=15, max.generations=10, replace=TRUE, cluster=c('localhost','localhost'))
mout4 <- Match(Y=y4, Tr=treat4, X=x4, estimand="ATT", Weight.matrix=genout4, replace=TRUE)
summary(mout4)

#Treatment effect on SubGroup: male  
male <- subset(nlsy, d_sexf=="0")
treat5 <- male$d_obese
x5 <- male[,c('d_sexf','childany','childf','age','d_urban_res',
                'srvy_yr','AFQTrevised','educ','tenure','NumberEmp',
                'd_race_b','d_race_o','d_marrnever','d_marroth',
                'd_ind_ag','d_ind_for','d_ind_mining','d_ind_const','d_ind_mfrg',
                'd_ind_transp','d_ind_wtrade','d_ind_rtrade','d_ind_finance',
                ...
                'd_occ_mgmt','d_occ_tech','d_occ_admin','d_occ_svc',
                'd_occ_farming','d_occ_prodxn','d_occ_operators')]
y5<- male$CPS_hourly_rec
genout5 <- GenMatch(Tr=treat5, X=x5, estimand="ATT", pop.size=10, max.generations=10, replace=TRUE, cluster=c('localhost','localhost'))
mout5 <- Match(Y=y5, Tr=treat5, X=x5, estimand="ATT", Weight.matrix=genout5, replace=TRUE)
summary(mout5)
