...
def aggregate_stats(tuple_list):
    ...
    new_list = [] #I want this new list to have the format as a list of sublists  
    for i in range(0,num_var):
        ...
        sub_list.append(my_mean(one_var)) #Calculate mean of data of each variable, append the value to sub_list
        sub_list.append(my_median(one_var)) #Calculate median of data of each variable, append the value to sub_list
        sub_list.append(my_mode(one_var)) #Calculate mode of data of each variable, append the value to sub_list
        sub_list.append(my_SD(one_var)) #Calculate standard deviation of data of each variable, append the value to sub_list
        new_list.append(sub_list) #Append sub_list to new_list so that in the end after the loop new_list contains a list of sublists, each sublist contains information about one variable
...
#so that I can use function my_cc for "cut" to calculate the correlation coefficient 
                ...
                new_l.append(l) #Append each small list to new_l
...
#Hacker Set: 
#Function two_variable: input: a tuple_list, output: a random list with data of two random variables 
...
# Function group_var, input is a list containing the data of two variables 
def group_var(list_two_var):
	print "Your list is: %s" % list_two_var
	print "The first variable is the one that we are doing statistics with"
	print "The second variable is the one that we are restricting on a specific range"
	low_end = int(raw_input('Enter the low_end value: '))
	high_end = int(raw_input('Enter the high_end value: '))
	new_value = []
	for i in range(1,len(list_two_var)):
		if list_two_var[i][1] in range(low_end, high_end+1):
			new_value.append(list_two_var[i][0])
	print new_value
	print "Mean of one variable when the other variable in range %s , %s is %s" % (low_end, high_end, my_mean(new_value))
	print "Median of one variable when the other variable in range %s , %s is %s" % (low_end, high_end, my_median(new_value))
	print "Mode of one variable when the other variable in range %s , %s is %s" % (low_end, high_end, my_mode(new_value))
	print "SD of one variable when the other variable in range %s , %s is %s" % (low_end, high_end, my_SD(new_value))


group_var(two_variable([('annual average temperature anomaly', 'annual precipitation anomaly', 'Palmer Drought Index anomaly', 'cooling degree days anomaly', 'heating degree days anomaly'), (-0.41, -0.24, 0.34, 22, 115), (-0.36, 0.39, 0.2, -18, 2), (-0.65, 1.76, 1.58, -57, 136), (0.27, 5.02, 4.45, 25, -223), (0.24, 0.68, 3.07, -81, -154), (-0.52, 3.09, 3.32, -34, -86), (-0.55, -3.63, -0.75, -173, 148), (0.53, 0.5, -1.55, 88, 14), (-0.97, 0.2, 0.52, 31, 360), (-1.14, 2.81, 1.83, -87, 192), (0.37, -1.67, -0.8, 110, 101), (1.1, 0.02, -1.16, 9, -92), (-0.67, 3.92, 2.97, -58, 17), (-0.14, 4.82, 4.61, 48, 19), (-0.04, 1.46, 3.55, 17, -105), (-0.72, 0.03, 0.56, 7, 23), (1.3, 1.44, 1.74, 59, -344), (1.31, -0.93, -0.97, 72, -296), (0.61, -4.04, -3.91, 91, -8), (-0.18, -0.89, -1.25, -27, 47), (1.49, 2.23, -0.37, 76, -619), (1.14, 2.5, 0.75, 140, -445), (0.58, 1.32, 0.79, -135, -228), (-0.76, 2.68, 3.97, 23, 38), (0.85, 0.68, 1.74, 38, -191), (0.63, 2.75, 2.4, 103, -150), (-0.13, 3.76, 2.39, 8, 13), (0.18, 1.92, 3.9, -25, -138), (2.21, 3.95, 1.15, 236, -684), (1.86, -1.47, -0.54, 117, -516), (1.25, -1.72, -4.84, 70, -247), (1.68, -0.92, -3.27, 86, -470), (1.19, -0.89, -2.46, 190, -360), (1.24, 0.57, 0.18, 91, -175), (1.08, 3.31, 0.91, 32, -361), (1.62, 0.14, -0.31, 199, -349), (2.23, -0.12, -2.62, 172, -648), (1.63, -0.76, -2.05, 199, -412), (0.27, 1.3, -0.05, 85, -181), (0.37, 2.36, 0.85, 44, -189), (0.96, 1.43, 2.65, 256, -198), (1.16, 0.16, -0.56, 268, -348), (3.26, -2.41, -4.41, 291, -891), (0.41, 1.12, -0.62, 100, -178), (0.51, 0.9, 0.84, 85, -80)]))
